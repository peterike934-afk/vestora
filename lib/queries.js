...
const LOCALE_COLUMN_MAP = {
  "es-MX": "es_mx",
}; // any locale not listed here just uses its code directly (ar, de, en, es, ru, zh)

export async function getInvestmentPlans(locale = "en") {
  const supabase = createClient();
  const { data, error } = await supabase
  ...
  const col = LOCALE_COLUMN_MAP[locale] || locale;

  return data.map((plan) => ({
    ...plan,
    name: plan[`name_${col}`] || plan.name_en,
    description: plan[`description_${col}`] || plan.description_en,
  }));
}
...
export async function updatePreferredLocale(userId, locale) {
  const supabase = createClient()
  const { error } = await supabase
    .from('profiles')
